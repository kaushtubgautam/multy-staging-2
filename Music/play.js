...
async execute(message, args, client) {
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Searching for 🔍 [\`LINK\`](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Searching for 🔍 \`${args.join(" ")}\`**`))
        }
      } else {
    ...
    //get the thumbnail
    let thumb = "https://media.discordapp.net/attachments/850619329628471336/866186031841411103/9e03a65f07f0efdebbe291fccd718f4d.gif?width=624&height=468"
    if (song.thumbnail === undefined) thumb = "https://media.discordapp.net/attachments/850619329628471336/866186031841411103/9e03a65f07f0efdebbe291fccd718f4d.gif?width=624&height=468";
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        ...
        .addField("Queue Number", `**\`${serverQueue.songs.length - 1}\`**`, true)
        .setFooter(`Queued by user: ${message.author.username}#${message.author.discriminator}`, message.member.user.displayAvatarURL({ dynamic: true }))
